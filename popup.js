...
function initializeTable() {
    ...
    tcpacDisplayDiv.insertAdjacentHTML("afterbegin", '<table><tr id="tcpacTableRow0"><th>Channel</th><th>Claimed Count (Points)</th><th>Delete Entry</th></tr></table>');
    chrome.storage.local.get('tcpacObj', function (result) {
        ...
        if(Object.keys(tcpacObj).length > 0) {
            for(var key in tcpacObj) {
                tableFirstRow.insertAdjacentHTML("afterend", "<tr id='tcpacTableRow"+(tableRowNum+1)+"'><td><a id='tcpacTableKey"+(tableRowNum+1)+"' href='https://www.twitch.tv/"+key+"' target='_blank'>"+key+"</a></td><td>"+tcpacObj[key]+" ("+(tcpacObj[key]*50)+")</td><td><button class='tcpacDelBtn' id='tcpacDelBtn"+(tableRowNum+1)+"'>Delete</button></td></tr>");
                tableRowNum += 1;
        ...
        // button
        tableFirstRow.insertAdjacentHTML("afterend", "<tr id='tcpacTableRowLast'><td></td><td></td><td><button id='tcpacRestBtn'>Delete All</button></td></tr>");
